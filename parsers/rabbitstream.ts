...
export default async function (main_arg_embed_url: string, main_arg_site: string) {
...
const getMeta = async (url: string) => {
    ...
    return await (async function(embed_url: string, site: string) {
        referrer = site;
        let xrax = embed_url.split("/").pop()?.split("?").shift();  //thanks itzzzme

        let regx = /https:\/\/[a-zA-Z0-9.]*/;
        let base_url = embed_url?.match(regx)?.[0] || "https://flixhq.to";
        ...
        return resp_json;
    })(main_arg_embed_url, main_arg_site);

}

// main("https://zizicoi.online/v2/embed-4/QM4U19kPqLWl?autoPlay=0", "https://vidstream.to"); //change this value to the embed_url you want
